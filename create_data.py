...
def get_summary_bstate(bstate, get_domain=False):
    ...
    if get_domain:
        ...
        # print('summary_bstate,summary_bvalue:',summary_bstate,summary_bvalue)
        # print('if len(summary_bstate)==len(summary_bvalue):', len(summary_bstate), len(summary_bvalue))
...
def analyze_dialogue(dialogue, maxlen):
    ...
    if len(d['log']) % 2 != 0:
        ...
        print('odd # of turns')
        return \
            None  # odd number of turns, wrong dialogue
    d_pp = {}
...
def divideData(data):
    ...
    trainListFile = open('data/trainListFile', 'w', encoding="utf8")
    ...
    # save all dialogues
    with open('data/dev_dials.json', 'w', encoding="utf8") as f:
        json.dump(val_dials, f, indent=4)

    with open('data/test_dials.json', 'w', encoding="utf8") as f:
        json.dump(test_dials, f, indent=4)

    with open('data/train_dials.json', 'w', encoding="utf8") as f:
